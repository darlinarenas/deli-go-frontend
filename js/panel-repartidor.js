document.addEventListener('DOMContentLoaded',async()=>{
  ...
  function renderPerfil(){
    const locked=profileEditing?'':'disabled';
    const field=(label,id,value)=>`<div class="field"><label>${label}</label><input id="${id}" value="${esc(value||'')}" ${locked}></div>`;
    q('#perfilView').innerHTML=`<div class="card profile-card"><div class="profile-heading"><div><span class="section-kicker">DATOS DEL REPARTIDOR</span><h2>Mi perfil</h2><p class="muted">${profileEditing?'Modifica únicamente los datos que necesites y luego guarda los cambios.':'Tus datos están protegidos. Presiona Editar datos para modificarlos.'}</p></div><span class="profile-mode ${profileEditing?'editing':''}">${profileEditing?'EDITANDO':'SOLO LECTURA'}</span></div><div class="form-grid profile-form">${field('Nombre','pName',session.fullName)}${field('Teléfono','pPhone',session.phone)}${field('Dirección','pAddress',session.address)}${field('Ciudad','pCity',session.city)}${field('Zona','pZone',session.zone)}${field('Vehículo','pVehicle',session.vehicleType)}${field('Marca','pBrand',session.vehicleBrand)}${field('Modelo','pModel',session.vehicleModel)}${field('Placa','pPlate',session.vehiclePlate)}${field('Color','pColor',session.vehicleColor)}</div>${profileEditing?`<div class="profile-actions"><button class="btn btn-dark" id="cancelProfileEdit" type="button">Cancelar</button><button class="btn btn-green" id="saveProfile" type="button">Guardar cambios</button></div>`:`<button class="btn btn-green btn-block" id="editProfile" type="button">Editar datos</button>`}</div>`
  }
