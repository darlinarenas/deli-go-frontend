document.addEventListener('DOMContentLoaded',async()=>{
  ...
  function jobCard(j,active=false){return `<article class="job-card"><div class="job-top"><div><span class="badge">${j.source_type==='FOOD_ORDER'?'PEDIDO DE RESTAURANTE':'ENVÍO DE PAQUETE'}</span><h3>${esc(j.pickup_name||'Punto de retiro')}</h3></div><strong>${money(j.driver_earning,j.currency)}</strong></div><div class="job-route"><div class="route-row"><i class="route-dot"></i><div><strong>Retiro</strong><small>${esc(j.pickup_address||'Dirección pendiente')}${j.pickup_reference?` · ${esc(j.pickup_reference)}`:''}</small></div></div><div class="route-row"><i class="route-dot"></i><div><strong>Entrega</strong><small>${esc(j.delivery_address||'Dirección pendiente')}${j.delivery_reference?` · ${esc(j.delivery_reference)}`:''}</small></div></div></div><div class="job-meta"><div class="mini-box"><span>Distancia</span><b>${Number(j.distance_km||0).toFixed(1)} km</b></div><div class="mini-box"><span>Total</span><b>${money(j.service_total,j.currency)}</b></div><div class="mini-box"><span>Estado</span><b>${esc(statusEs(j.status))}</b></div></div><div class="action-row">${active?activeButtons(j):`<button class="btn btn-green" data-accept-job="${j.id}">Aceptar entrega</button>`}<button class="btn btn-dark" data-map="${encodeURIComponent(active&&['PICKED_UP','GOING_TO_DELIVERY','ARRIVED_AT_DELIVERY'].includes(j.status)?j.delivery_address||'':j.pickup_address||'')}">Abrir en mapa</button></div></article>`}
