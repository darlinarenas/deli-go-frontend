...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function handleCheckoutSubmit(event) {
    ...
    if (isInviteOrder) {
      try {
        generatedInvite = await createDeliveryInvite(created.id || order.id, order);
        showGeneratedInviteLink(generatedInvite);
      } catch (error) {
        console.error("No se pudo generar invitación BHUZ:", error);
        alert("El pedido se creó, pero no se pudo generar el link de invitación. Revisa el backend antes de continuar.");
        return;
      }
    }
    ...
    if (isInviteOrder) {
      alert("Pedido creado. Ahora comparte el link con la persona invitada para que confirme su ubicación.");
      return;
    }
  ...
  refreshSavedGuestsBtn?.addEventListener("click", loadSavedInviteGuests);
